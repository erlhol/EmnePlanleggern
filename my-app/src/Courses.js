...
function FilterButtons(props) {
  ...
  const start = 1;
  const end = 9;
  const years = Array.from({ length: end - start + 1 }, (_, index) =>
  ...
  const filterCredits = (subjects, low, high) => {
    return subjects.filter(
      courseObj => courseObj.credits >= low && courseObj.credits <= high
    );
  };
  ...
  const filterExam = subjects => {
    return subjects.filter(
      courseObj => courseObj.grading !== null && courseObj.grading === false
    );
  };
  ...
  const filterLanguage = (subjects, filter) => {
    ...
    return subjects.filter(
      courseObj =>
        courseObj.teachingLanguage !== undefined &&
        courseObj.teachingLanguage.includes(filter)
    );
  };

  const filterYears = (subjects, filter) => {
    if (filter.length === 0) return subjects;
  ...
  return (
    <div className="wrapping">
      <div className="filter_row">
        ...
        <div>
          <label>
            <input onChange={onPassFailChange} type="checkbox" />
            Pass/fail
          </label>
        </div>
        {slider}
        ...
        <div>
          ...
          {years.map(lvl => (
            <div key={lvl} style={{ display: "flex", flexDirection: "column" }}>
              <label>
                <input onChange={() => onSelectedYears(lvl)} type="checkbox" />
                {lvl}000
              </label>
            </div>
          ))}
...
function Course(props) {
  ...
  return (
    <div className={"course"}>
      ...
      <button onClick={() => console.log("Open new webpage")}>
        Learn more
      </button>
      <button
        onClick={() =>
          window.open(
            `https://www.karakterweb.no/uio/${props.courseObject.subjectCode.toLowerCase()}`,
            "_blank",
            "noreferrer"
          )
        }
    ...
    <div>
      ...
      <div className="split_view">
        ...
        <FilterButtons
          ...
          changeRetrieved={setRetrievedSubjects}
